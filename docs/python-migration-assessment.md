# Python Migration Assessment

- **Date:** 2026-04-25
- **Author:** Joo Wook Ahn
- **Reviewer:** Becky Locke, Matt Garner
- **Confluence:** https://cuhbioinformatics.atlassian.net/wiki/spaces/DV/pages/4673994824

---

**Current stack:** Next.js 15 (App Router) + TypeScript, ECS Fargate, RDS PostgreSQL, Lambda, S3  
**Assessment scope:** Feasibility and effort of converting to a Python web framework with maintainability and team expertise as priorities

---

## 1. What the Application Currently Does

Before evaluating the migration, it is worth understanding how Next.js is being used.

Next.js is doing double duty:
...
**The classification engine is a particularly good candidate** — the ACGS SNV and SVIG-UK scoring algorithms are pure numeric logic over JSON-defined rules. The JSON config files (`acgs-snv-criteria.json`, `svig-criteria.json`) are framework-agnostic and carry over unchanged.
...
Faithfully replicating this in HTMX with server-round-trips on every criterion toggle is achievable but would require careful design of partial-update endpoints and would likely result in slightly degraded interactivity compared to the current React-driven live scoring.
...
_Estimates assume a single developer with strong Python skills and working familiarity with the codebase. NHS team working patterns (e.g. part-time capacity, clinical commitments) will extend calendar time accordingly._
...
- **Two language runtimes remain.** The team still needs to maintain TypeScript/JavaScript knowledge for the React frontend.
...
- **Single language runtime.** Python end-to-end — no JavaScript build tooling, no TypeScript, no npm.
...
- **Highest up-front effort.** The ClassificationPanel rewrite is the dominant risk. Live criterion toggling with immediate score feedback is difficult to replicate idiomatically in HTMX without careful endpoint design.
- **HTMX is a less mature ecosystem for complex data UIs.** The variant table (sortable columns, client-side filters, paginated server fetch) requires more backend endpoints compared to the current client-driven approach.
- **Jinja2 templates are harder to test than React components.** Unit-testing template rendering requires integration tests — more setup than React Testing Library.
...
- **"Batteries included" framework.** Django provides an ORM, a migration framework, a built-in admin interface, session management, and a mature authentication system.
- **Django ORM reduces SQL maintenance burden.** Raw parameterised SQL is replaced by querysets that are easier to read and refactor.
...
- **Highest learning curve.** Django introduces significantly more concepts than Flask or FastAPI: ORM, migrations, settings modules, apps, middleware, class-based views.
- **ORM migration from raw SQL is not free.** The existing SQL queries (particularly the lateral join in `variants` and the transactional classification logic) need careful mapping.
- **Async support is incomplete.** Django's async support (as of Django 5.x) is usable but not as mature as FastAPI's.
...
1. **Risk is concentrated in the right place.** The business logic and API routes — where Python expertise is most valuable — are fully migrated. The React UI, which currently works correctly, is preserved.
2. **The frontend is already thin.** The React components contain almost no Next.js-specific code. Migration is closer to a scaffolding task than a rewrite.
3. **The team's Python skills directly improve the most clinically important code.** The classification engine (ACGS SNV, SVIG-UK scoring), VCF parser, and ingest pipeline are the core of the application's clinical value.
4. **Option A can evolve into Option B incrementally.** Once the Python backend is stable, individual React components can be replaced with HTMX partials over time — without a big-bang rewrite.
5. **Option A is compatible with NHS governance timelines.** A 4–6 week backend rewrite can be scoped as a single sprint or quarter of work.
